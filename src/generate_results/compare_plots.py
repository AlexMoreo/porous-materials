...
methods = [#'RFY', #'1NN',  #'AE-ZY',
           ...
           'PAE2ZY', 'PAE2ZY-com', 'PAEZY', 'PAEZY-com'
           ]


# --- selected models to display ---
from experiments import load_data
path_h2 = '../../data/incomplete/training_set_with_exp_H2.csv'
path_n2 = '../../data/incomplete/training_set_with_exp_N2.csv'
test_names, *_ = load_both_data(
    path_input_gas=path_n2, path_output_gas=path_h2, cumulate_vol=True, normalize=True,
    return_index=True, exclude_id=[]
)
all_ids = test_names
...
methods_path = join(result_dir, 'plots')
for gas_suffix in ['Vin', 'Gout']:
    if gas_suffix == 'Gout':
        methods = ['RFY'] + methods
    print(f'Generating plots for {gas_suffix}')
    gas_path = join(result_dir, f'plots-{gas_suffix}')
