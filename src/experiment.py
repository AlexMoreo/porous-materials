...
def methods():
    ...
    yield 'transformer-5L', NeuralRegressor(TransformerRegressor(input_size, output_size, num_layers=5), clip=True)
...
for i, (train, test) in enumerate(loo.split(X, y)):
    ...
    for method, reg in methods():
        print(f'{method=}')
        show_table = False
        method_errors = ResultTracker(f'../results/errors/{gas}/{method}.pkl')
        method_convergence = ResultTracker(f'../results/convergence/{gas}/{method}.pkl')
